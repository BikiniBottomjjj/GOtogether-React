...
export default function MapSearchSection({ onAdd }: Props) {
    ...
    const openNaverMap = () => {
        ...
        const deepLink = keyword
            ? `nmap://search?query=${encodeURIComponent(keyword)}&appname=com.gotogether.app`
            : `nmap://`
        const webLink = keyword
    ...
    const openKakaoMap = () => {
        const keyword = query.trim()
        const deepLink = keyword
            ? `kakaomap://search?q=${encodeURIComponent(keyword)}`
            : `kakaomap://`
        const webLink = keyword
    ...
    return (
        <div style={{ padding: '10px 12px' }}>
            ...
            {/* 지도 */}
            <div ref={mapRef} style={{ height: 200, borderRadius: 8, marginBottom: 0, background: '#deecd8' }} />
            ...
            <div style={{ display: 'flex', gap: 8, margin: '8px 0' }}>
                <button
                    ...
                    style={{
                        flex: 1, padding: '10px 0', background: '#03C75A',
                ...
                <button
                    ...
                    style={{
                        flex: 1, padding: '10px 0', background: '#FEE500',
                        color: '#3C1E1E', border: 'none', borderRadius: 8,
                        fontSize: 13, fontWeight: 600, cursor: 'pointer'
                    }}
                >카카오 지도 바로가기</button>
            </div>
